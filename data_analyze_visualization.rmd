...
data_HPC_S <- read.table("static_setonix.tsv", header = TRUE)
data_HPC_D <- read.table("dynamic_setonix.tsv", header = TRUE)
...
combined_data <- bind_rows(data_HPC_S, data_HPC_D)
...
ggplot(thread_averages, aes(x = Num_Threads, y = Avg_Total_Time, color = Source)) +
  ...
  labs(x = "Number of Threads", y = "Average Total Time",
       title = "Average Total Time vs. Number of Threads (Static vs. Dynamic)",
...
# Read the CSV file
data_ReductionCritical <- read.csv("Reduction_Critical.csv")
...
plot <- ggplot(data_ReductionCritical, aes(x = Threads, y = Runtime, color = Construct)) +
  ...
  scale_x_log10() +
  labs(x = "Threads", y = "Runtime", title = "Runtime vs. Threads by Construct") +
  theme_minimal() +
  theme(
...
# Read the CSV file
data_strategy <- read.csv("strategies.csv")
...
ggplot(combined_data, aes(x = as.factor(Thread), y = Time, fill = Section)) +
  ...
  labs(x = "Number of Threads", y = "Execution Time", title = "Comparison of Reduction vs. Critical Sections") +
  theme_minimal() +
  theme(legend.position = "top") +
...
ggplot(data, aes(x = construct, y = total_time, fill = factor(thread_num))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Construct", y = "Total Time", title = "Total Time by Construct and Thread Number") +
